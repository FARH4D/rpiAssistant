...
############################################################################################################
        ...
        if datetime.today().date() > datetime.strptime(timesParsed["date"], '%Y-%m-%d').date():
            ...
            if "error" in data:
                ...
                errorFrame = customtkinter.CTkFrame(master, width=250, height=45)
                errorFrame.pack(side='top', pady=15)
                errorFrame.grid_propagate(False)

                errorLabel = customtkinter.CTkLabel(errorFrame, text = "ERROR. API CALL FAILED.", font = ("Roboto", 18))
                errorLabel.grid(row=0, column = 0, padx = (10, 0), pady=9)
